...
class CoreViewTests(TestCase):
    ...
    def test_progress_create_view_valid_post(self):
        ...
        self.assertEqual(response.status_code, 302)
        self.assertTrue(ProgressUpdate.objects.filter(title='New Progress', user=self.user).exists())
    ...
    def test_progress_delete_by_owner(self):
        ...
        response = self.client.post(url)
        self.assertEqual(response.status_code, 302)
        self.assertFalse(ProgressUpdate.objects.filter(pk=self.progress.pk).exists())
    ...
    def test_newsletter_subscribe_invalid_email(self):
        response = self.client.post(reverse('core:newsletter_subscribe'), {'email': 'not-an-email'})
        self.assertEqual(response.status_code, 302)
